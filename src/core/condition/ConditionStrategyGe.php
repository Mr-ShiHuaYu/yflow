...
/**
 * ConditionStrategyGe - 条件表达式大于等于 ge@@flag|4
 *
 *
 */
class ConditionStrategyGe extends AbstractConditionStrategy
{
    ...
    public function afterEval(string $value, string $variableValue): ?bool
    {
        if (MathUtil::isNumeric($value)) {
            $result = MathUtil::determineSize($variableValue, $value);
